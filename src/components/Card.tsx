	...
	>
		...
		<div className="p-6 md:p-8 h-full flex flex-col relative z-10">
			<div className="flex justify-between items-center mb-6 border-b-2 border-black/10 pb-4">
				...
				{image ? (
					<img
						src={image}
						alt="Logo"
						className="h-10 md:h-14 max-w-32 object-contain -mt-4"
					/>
				) : Icon ? (
			...
			<div className="mt-4 flex justify-between items-end">
				<Barcode className="opacity-40" seed={typeof title === "string" ? title : "default"} />
				<div className="magnetic relative z-50">
					{link ? (
						<a href={link} target="_blank" rel="noopener noreferrer">
							<ArrowUpRight
								size={32}
								className="text-black transition-transform duration-300 group-hover:rotate-45 relative z-50 cursor-pointer"
							/>
						</a>
					) : (
						<ArrowUpRight
